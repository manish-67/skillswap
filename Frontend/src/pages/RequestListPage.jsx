...
const RequestListPage = () => {
  ...
  const fetchRequests = async (searchParams) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const handleSearchSubmit = (e) => {
    ...
    navigate(`/requests?${new URLSearchParams(params).toString()}`);
  };
  ...
  return (
    <div style={{ padding: '20px', minHeight: '100vh', background: 'linear-gradient(120deg, #e3f0ff 0%, #f8f9fa 100%)' }}>
      ...
      >
        <input
          type="text"
          placeholder="Search by keyword (e.g., Python, proofreading)"
          value={keyword}
        ...
        <button
          type="button"
          onClick={() => { setKeyword(''); setSelectedCategory(''); setLocationFilter(''); navigate('/requests'); }}
          style={{
            padding: '10px 18px',
      ...
      {loading ? (
        ...
        <p style={{ textAlign: 'center' }}>No requests found matching your criteria.</p>
      ) : (
        <div
          style={{
            display: 'grid',
          ...
          {requests.map((request) => (
            ...
            >
              <h3 style={{ margin: '0 0 10px 0', color: '#007bff', fontWeight: 600 }}>
                <Link to={`/requests/${request._id}`} style={{ textDecoration: 'none', color: '#007bff' }}>
                  {request.title}
                </Link>
              </h3>
              <p style={{ fontSize: '0.97em', color: '#555', margin: '0 0 8px 0' }}>
                By: {request.user ? request.user.name : 'Unknown'} ({request.user ? request.user.location : 'N/A'})
              </p>
